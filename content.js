(function() {
    ...
    const GM_xmlhttpRequest = (options) => {
        if (extAPI && extAPI.runtime && extAPI.runtime.sendMessage) {
            extAPI.runtime.sendMessage({ action: "fetchBlob", url: options.url, referer: window.location.href })
                .then(response => {
    ...
    // Inject external interceptor to bypass Chrome CSP
    function injectInterceptor() {
        if (document.getElementById('comic-dl-interceptor')) return;
        const scriptEl = document.createElement('script');
        scriptEl.id = 'comic-dl-interceptor';
        scriptEl.src = extAPI.runtime.getURL('inject.js');
